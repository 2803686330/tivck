...
import { TDrawLine, TRandom, TDot } from '../type';
function Qcanvas() {
  useEffect(() => {
      ...
      const draw = () => {
        ...
        dotList.forEach((dot, index) => {
          ...
          for (let i = index + 1; i < dotList.length; i++) {
            ...
            // 计算两个小球的x 与 y 坐标差值
            const x_dist = dot.x - nextDot.x;
            const y_dist = dot.y - nextDot.y;
          ...
          if (point.x !== -1) {
            // 计算鼠标与当前小球坐标差值
            const x_dist = point.x - dot.x;
            const y_dist = point.y - dot.y;
            // 计算鼠标与当前小球直线距离
            const dist = x_dist * x_dist + y_dist * y_dist;
            // 小于预设值（可以连线）
            if (dist < distSquare) {
